...
// --- Get counts for the top display cards ---
$problemTypeCounts = [];
$sql_counts = "SELECT problem_type, status, COUNT(id) as count FROM po_issues WHERE reported_by_empcode = ? GROUP BY problem_type, status";
$stmt_counts = $conn_po->prepare($sql_counts);
if($stmt_counts) {
    $stmt_counts->bind_param("s", $empcode);
    $stmt_counts->execute();
...
$filterColumns = [
    'status', 'problem_type', 'site', 'supplier_id', 'gate_entry_no', 'invoice_no', 'challan_no', 'po_number', 'project_id', 'buyer_id','buyer_name', 'reported_by_empcode', 'assignee_empcode','assignee_name', 'user_confirmation'];
...
// --- Build WHERE clause ---
$whereClauses = ["reported_by_empcode = ?"];
$params = [$empcode];
$types = "s";
...
$mainWhereSql = " WHERE " . implode(" AND ", $whereClauses);

// --- Fetch Cascading Filter Options ---
$filterOptions = [];
foreach ($filterColumns as $columnToQuery) {
    $cascadingWhereClauses = ["reported_by_empcode = ?"];
    $cascadingParams = [$empcode];
    $cascadingTypes = "s";
    ...
    $cascadingWhereSql = " WHERE " . implode(" AND ", $cascadingWhereClauses);
