<?php
// page name - track_your_issues.php
...
<html lang="en">
  <head>
    ...
    <title>Track Your Issues - SIMPLEX INTERNAL PORTAL</title>
  ...
  <body>
        ...
        <div class="sidebar-wrapper scrollbar scrollbar-inner"><div class="sidebar-content"><ul class="nav nav-secondary">
          ...
          <li class="nav-item active submenu">
            ...
            <div class="collapse show" id="poIssuesCollapse"><ul class="nav nav-collapse">
              ...
              <li class="active"><a href="track_your_issues.php"><span class="sub-item">Track Your Issues</span></a></li>
      ...
      <div class="main-panel">
        ...
        <div class="container">
          <div class="page-inner">
            <h1 class="page-header-title mt-4">Track Your Raised Issues</h1>
        ...
        <i class="fas fa-arrow-left me-2"></i>Back to Dashboard
            ...
            <div class="card">
                ...
                <div class="collapse show" id="filterCollapse"><div class="card-body"><div class="row g-3" id="dynamic-filters">
                    ...
                    <div class="col-lg-3 col-md-6"><label class="form-label">Gate Entry Number</label><select class="form-select filter-control" data-filter-key="gate_entry_no"></select></div>
                    <div class="col-lg-3 col-md-6"><label class="form-label">Status</label><select class="form-select filter-control" data-filter-key="status"></select></div>
                    ...
                    <div class="col-lg-3 col-md-6"><label class="form-label">PO Number</label><select class="form-select filter-control" data-filter-key="po_number"></select></div>
                    ...
                    <div class="col-lg-3 col-md-6"><label class="form-label">Creation Date (From-TO)</label><div class="input-group"><input type="date" class="form-control filter-control" data-filter-key="created_at_from"><input type="date" class="form-control filter-control" data-filter-key="created_at_to"></div></div>
                    <div class="col-lg-3 col-md-6"><label class="form-label">Assignee Name</label><select class="form-select filter-control" data-filter-key="assignee_name"></select></div>
                </div></div></div>
            </div>
            <div class="card">
    ...
    $(document).ready(function() {
      ...
      const tableHeaders = ['ID (Track History of Issue)', 'Status','User Confirmation', 'Issue raised At','Site', 'Problem Type', 'Gate Entry No', 'Supplier ID', 'Supplier Name', 'Invoice Number', 'Invoice Date', 'Challan Number', 'Challan Date', 'PO Number',  'Project ID', 'Buyer ID', 'Buyer Name', 'Attachment Path',  'Assigned To ',  'Assigned At' ];
      const dbKeys = ['id', 'status', 'user_confirmation', 'created_at','site', 'problem_type', 'gate_entry_no', 'supplier_id', 'supplier_name', 'invoice_no', 'invoice_date', 'challan_no', 'challan_date', 'po_number',  'project_id', 'buyer_id', 'buyer_name', 'attachment_path', 'assignee_name',  'status_set_by_assignee_at'];
        ...
        function fetchAndRender() {
            ...
            $.ajax({
                url: 'fetch_my_issues.php', type: 'GET', dataType: 'json', data: { ...filters, page: currentPage },
       ...
       function updateTable(issues) {
            ...
            if (issues.length === 0) {
                tableBody.html(`<tr><td colspan="${tableHeaders.length}" class="text-center">No issues found.</td></tr>`);
                return;
            }
            ...
            issues.forEach(issue => {
                let row = '<tr>';
                dbKeys.forEach(key => {
                    let cellData = issue[key] ?? 'N/A'; // Default value if data is null

                    // --- START OF LOGIC ---
                    if (key === 'id') {
                        cellData = `<a href="#" class="issue-log-link" data-issue-id="${issue.id}">${issue.id}</a>`;
                    } 
                    else if (key === 'status') {
                    ...
                    else if (key === 'user_confirmation') {
                        // THIS IS THE CORRECTED LOGIC BLOCK
                        if (issue.status === 'Resolved' && issue.user_confirmation === null) {
                            cellData = `<div class="btn-group btn-group-sm">
                                            <button class="btn btn-success confirmation-btn" data-issue-id="${issue.id}" data-confirmation="1">Yes</button>
                                            <button class="btn btn-danger confirmation-btn" data-issue-id="${issue.id}" data-confirmation="0">No</button>
                                        </div>`;
                        } else if (issue.user_confirmation == '1') {
                            cellData = '<span class="badge bg-success">Confirmed</span>';
                        } else if (issue.user_confirmation == '0') {
                            cellData = '<span class="badge bg-danger">Re-Opened</span>';
                        } else {
                            cellData = 'N/A';
                        }
                    } 
                    else if (key === 'attachment_path') {
                        ...
                        cellData = issue[key] ? new Date(issue[key]).toLocaleDateString() : 'N/A';
                    } 
                    else if (key.includes('remarks')) {
                        cellData = `<span title="${issue[key] || ''}">${(issue[key] || 'N/A').substring(0, 20)}...</span>`;
                    }
                    else if (key === 'actions') {
                        cellData = `<a href="#" class="btn btn-sm btn-info issue-log-link" data-issue-id="${issue.id}">View Log</a>`;
                    }
                    // --- END OF LOGIC ---
        ...
        $(document).on('click', '.confirmation-btn', function() {
            const issueId = $(this).data('issue-id'); const confirmationStatus = $(this).data('confirmation');
            const actionText = confirmationStatus == 1 ? "close this issue" : "re-open this issue";
            Swal.fire({
                title: 'Confirm Resolution', text: `Please provide remarks for your decision.`, input: 'textarea',
                inputPlaceholder: 'Type your remarks here...', showCancelButton: true, confirmButtonText: `Submit & ${confirmationStatus == 1 ? 'Close' : 'Re-Open'}`,
                preConfirm: (remarks) => { if (!remarks) { Swal.showValidationMessage(`Remarks are required.`); } return remarks; }
            }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'confirm_resolution.php', type: 'POST', dataType: 'json',
                        data: { issue_id: issueId, confirmation_status: confirmationStatus, resolution_remarks: result.value },
                        success: (response) => {
                            if (response.status === 'success') { Swal.fire('Success!', response.message, 'success'); fetchAndRender(); }
                            else { Swal.fire('Error!', response.message, 'error'); }
                        },
