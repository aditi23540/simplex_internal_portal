...
<html lang="en">
  <head>
    ...
    <style>
      ...
      .card-stats .card-title { font-size: 1rem; margin-bottom: 0.5rem; font-weight: 600; }
        ...
        <div class="sidebar-wrapper scrollbar scrollbar-inner"><div class="sidebar-content"><ul class="nav nav-secondary">
          ...
          <li class="nav-item active submenu">
            ...
            <div class="collapse show" id="poIssuesCollapse"><ul class="nav nav-collapse">
              ...
              <li class="active"><a href="all_issues.php"><span class="sub-item">All Issues</span></a></li>
            </ul></div>
          </li>
        </ul></div></div>
      </div>
...
<!-- End Back to Dashboard Button -->
            ...
            <div class="card">
                ...
                <div class="collapse show" id="filterCollapse"><div class="card-body"><div class="row g-3" id="dynamic-filters">
                    ...
                    <div class="col-lg-3 col-md-6"><label class="form-label">Reporter</label><select class="form-select filter-control" data-filter-key="reported_by_empcode"></select></div>
                    <div class="col-lg-3 col-md-6"><label class="form-label">Assignee</label><select class="form-select filter-control" data-filter-key="assignee_empcode"></select></div>
                    ...
                    <div class="col-lg-3 col-md-6"><label class="form-label">Buyer Name</label><select class="form-select filter-control" data-filter-key="buyer_id"></select></div>
                    <div class="col-lg-3 col-md-6"><label class="form-label">Creation Date</label><div class="input-group"><input type="date" class="form-control filter-control" data-filter-key="created_at_from"><input type="date" class="form-control filter-control" data-filter-key="created_at_to"></div></div>
                    <div class="col-lg-3 col-md-6"><label class="form-label">Updated Date</label><div class="input-group"><input type="date" class="form-control filter-control" data-filter-key="updated_at_from"><input type="date" class="form-control filter-control" data-filter-key="updated_at_to"></div></div>
                    <div class="col-lg-3 col-md-6"><label class="form-label">Invoice Date</label><div class="input-group"><input type="date" class="form-control filter-control" data-filter-key="invoice_date_from"><input type="date" class="form-control filter-control" data-filter-key="invoice_date_to"></div></div>
                    <div class="col-lg-3 col-md-6"><label class="form-label">Challan Date</label><div class="input-group"><input type="date" class="form-control filter-control" data-filter-key="challan_date_from"><input type="date" class="form-control filter-control" data-filter-key="challan_date_to"></div></div>
                </div></div></div>
            </div>
                     <div class="d-flex justify-content-end mb-3">
...
            </div>
    ...
    $(document).ready(function() {
       ...
       const tableHeaders = ['ID (Track History)','Status','Site', 'Reporter','Assignee', , 'Problem', 'Supplier ID', 'Supplier Name','Buyer ID','Buyer Name', 'Attachment', 'PO No', 'Invoice Number', 'Invoice Date', 'Challan Number', 'Challan Date', 'Project ID' ];
        const dbKeys = ['id','status', 'site', 'reported_by_name', 'assignee_name', 'problem_type','supplier_id', 'supplier_name','buyer_id', 'buyer_name', 'attachment_path', 'po_number', 'invoice_no','invoice_date','challan_no', 'challan_date', 'project_id' ];
    ...
    cellData = issue.attachment_path 
        ...
          // START: New Export Button Logic
        $(document).on('click', '#export-excel-btn', function(e) {
            e.preventDefault(); // Prevent default link behavior

            // The 'filters' object is already kept up-to-date by the fetchAndRender function.
            // We can use it directly to build the query string.
            const queryParams = new URLSearchParams(filters).toString();
            
            // Construct the final URL and trigger the download
            const exportUrl = `export_all_issues_excel.php?${queryParams}`;
...
        // END: New Export Button Logic
        ...
        $(document).on('change', '.status-update-dropdown', function() {
            ...
            if (!newStatus) return;
            Swal.fire({ title: 'Confirm Status Change', text: `Please provide remarks for this action.`, input: 'textarea',
