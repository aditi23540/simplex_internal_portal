...
$filterColumns = [
    'problem_type', 'site', 'supplier_id', 'gate_entry_no', 'invoice_no', 'challan_no', 
    'po_number', 'project_id', 'buyer_id', 'reported_by_empcode', 'assignee_empcode'
];
$dateFilters = ['invoice_date', 'challan_date', 'created_at', 'updated_at'];
$activeFilters = [];
...
// --- Build WHERE clause ---
$whereClauses = ["status = 'Closed'"]; // Hardcoded filter for this page
$params = [];
$types = "";
...
foreach ($filterColumns as $columnToQuery) {
    $cascadingWhereClauses = ["status = 'Closed'"];
    $cascadingParams = [];
    $cascadingTypes = "";
...
echo json_encode([
    "status" => "success", "data" => $issues,
    "pagination" => [ "currentPage" => $page, "totalPages" => $totalPages, "totalRecords" => $totalRecords ],
    "problemTypeCounts" => $problemTypeCounts, "filterOptions" => $filterOptions
]);
$conn_po->close();
$conn_user->close();
?>