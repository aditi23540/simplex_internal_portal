<?php
// /edit_user_form.php (Single Page, Complete Version)
...
// 2. CSRF Token Generation for Security
...
require_once 'includes/db_config.php'; // Main DB Connection

// --- Avatar Fetching (Using main connection for efficiency) ---
$avatar_path = "assets/img/kaiadmin/default-avatar.png";
if ($empcode !== 'N/A' && isset($link)) {
    ...
    if ($stmt_avatar = $link->prepare($sql_avatar)) {
        ...
        if ($result_avatar->num_rows > 0) {
            ...
            $full_avatar_path = __DIR__ . "/../registration_project/" . $db_avatar_path;
            if (!empty($db_avatar_path) && file_exists($full_avatar_path)) {
...
// --- Logic to fetch ALL details for the specific user being edited ---
...
if (isset($_GET['user_id']) && filter_var($_GET['user_id'], FILTER_VALIDATE_INT)) {
    ...
    // This entire block fetches all necessary data to pre-populate the form.
...
if ($stmt_parents = mysqli_prepare($link, $sql_parents)) {
    mysqli_stmt_bind_param($stmt_parents, "i", $user_id);
    mysqli_stmt_execute($stmt_parents);
    $result_parents = mysqli_stmt_get_result($stmt_parents);

    // ✅ MOVED THE LOOP INSIDE THE SUCCESS BLOCK
    // This code now only runs if the query was successful and $result_parents is valid.
    ...
    // This will help you find SQL errors in the future.
    // You can remove this line after debugging.
    echo "Error preparing parents query: " . mysqli_error($link);
}
    ...
    $_SESSION['message'] = "Invalid User ID."; $_SESSION['message_type'] = "error"; header("Location: view_users.php"); exit();
}

// Helper function to safely echo values into form fields
function e($value) { return htmlspecialchars($value ?? '', ENT_QUOTES, 'UTF-8'); }

// Helper to display current file and provide a new upload field
function displayEditableFileLink($path, $input_name, $existing_path_name) {
    ...
    if (!empty($path)) {
        $web_path = "../registration_project/" . $path;
        if (file_exists(__DIR__ . "/" . $web_path)) {
             $fileName = basename(htmlspecialchars($path));
             $current_file = '<a href="' . $web_path . '" target="_blank" class="text-primary d-block mb-1 small">View Current: ' . $fileName . '</a>';
        } else {
            $current_file = '<span class="form-text text-danger small">Current file not found on server.</span>';
        }
    }
...
$page_specific_title = "Edit User Details";
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
        ...
        <div class="container-fluid">
            <div class="navbar-brand-wrapper d-flex align-items-center me-auto">
                <a href="index.php" style="display: flex; align-items: center; text-decoration: none; color: #333;">
                    ...
                    <span style="font-size: 1.5rem; font-weight: 500;"> Simplex Engineering and Foundry Works </span>
                </a>
            </div>
            <ul class="navbar-nav topbar-nav ms-md-auto align-items-center">
                <li class="nav-item topbar-user dropdown hidden-caret">
                    <a class="dropdown-toggle profile-pic" data-bs-toggle="dropdown" href="#" aria-expanded="false">
                        <div class="avatar-sm">
                            <img src="<?php echo e($avatar_path); ?>" alt="User Avatar" class="avatar-img rounded-circle" onerror="this.onerror=null; this.src='assets/img/kaiadmin/default-avatar.png';" />
                        </div>
                        <span class="profile-username">
                            <span class="op-7">Hi,</span> <span class="fw-bold"><?php echo $username_for_display; ?></span>
                        </span>
                    </a>
                    <ul class="dropdown-menu dropdown-user animated fadeIn">
                        <div class="dropdown-user-scroll scrollbar-outer">
                            <li>
                                <div class="user-box">
                                    <div class="avatar-lg"><img src="<?php echo e($avatar_path); ?>" alt="image profile" class="avatar-img rounded" onerror="this.onerror=null; this.src='assets/img/kaiadmin/default-avatar.png';"/></div>
                                                 <div class="u-text">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex align-items-center">
                                <h4 class="card-title fw-bold">Edit User: <?php echo e($user_data['first_name'] . ' ' . $user_data['surname']); ?></h4>
                                <a href="view_users.php" class="btn btn-secondary btn-round ms-auto"><i class="fas fa-arrow-left"></i> Back To Employee Master</a>
                            </div>
                        </div>
                        <div class="card-body">
                            <form action="actions/update_user.php" method="POST" enctype="multipart/form-data" id="editUserForm">
                                ...
                                <div class="card-footer text-center mt-4">
                                    ...
                                    <a href="view_user_details.php?user_id=<?php echo e($user_id); ?>" class="btn btn-danger btn-lg">Cancel</a>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <div class="container-fluid d-flex justify-content-between">
        <div class="copyright">
            <?php echo date('Y')?>, made with <i class="fa fa-heart heart text-danger"></i> by <a href="#">Abhimanyu</a>
        </div>
        <div>
